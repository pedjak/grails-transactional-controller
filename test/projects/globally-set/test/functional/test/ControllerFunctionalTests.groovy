...
class ControllerFunctionalTests extends functionaltestplugin.FunctionalTestCase {
    ...
    void testBControllerWithoutTransactionalFieldButGlobalSetToTrue() {
        // put one
        get("/b/put?value=v1")
        assertStatus 200
        ...
        // simulate error by update
        get("/b/put?value=v3&id=${id}&error=${Throwable.name}")
        assertStatus 500
        ...
        response = parser.parseText(page.content)
        assertEquals(1, response.b.size())
        assertEquals(id, response.b[0]['@id'])
        assertEquals("v2", response.b[0].value[0].text())
    }
    ...
    void testDControllerWithTransactionalFieldAsListOfActionNames() {
        ...
        // check if txstatus is available
        get("/d/rollback?value=v5
        assertStatus 200
        
        response = parser.parseText(page.content)
        assertEquals("v5", response.value.text())
        
        get("/d/list")
        assertStatus 200
        
        // the value was not inserted due to rollback
        response = parser.parseText(page.content)
        assertEquals(2, response.d.size())
        ["v2", "v4"].each { v ->
            assertEquals(1, response.d.grep { it.value.text() == v }.size())
